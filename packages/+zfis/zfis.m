function fis = zfis(data_train, n_mfs, epochs, data_test)
    % Zero Order ELANFIS
    %
    % Parameters
    % ----------
    ...
    % -------
    % `fis`
    %   the fuzzy inference system tuned using ELANFIS
    ...
    for e = 1 : epochs
        ...
        P = y_train * pinv(rule_mat);

        % Inverse with regularization
        %P = (eye(n_rules * (n_variables + 1)) / 10000 + rule_mat * rule_mat') \ rule_mat * y_train';

        output_params = [zeros(n_rules, n_variables), P'];
